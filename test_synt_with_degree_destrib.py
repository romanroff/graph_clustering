...
def generate_random_graph():
    ...
    while (handshake(degrees)):
        degrees = np.random.choice(range(0, len(W)), size=N, p=W)

    data = {i: degrees[i] for i in range(len(degrees))}
    ...
    for i in range(len(data)):
        G.add_node(i)
    nodes = [[i, int(data[i])] for i in data]
    for i in trange(len(nodes)):
        u, du = nodes[i]
        if du == 0:
            continue
        for j in range(len(nodes) - 1, len(nodes) - 1 - du, -1):
            nodes[j][1] -= 1
            G.add_edge(i, j, length=np.random.random() + 0.01)
        nodes[i][1] = 0
...
def calculate(data):
    ...
    tqdm.write(str(len(G.edges) / len(G.nodes)))
    ...
    for i in range(1):
        city_tests.test_graph(Q,
                              f'Synt_{len(G.nodes)}_{len(G.edges)}_{nx.density(Q):.5f}',
                              '0',
...
if __name__ == '__main__':
    ...
    #     res = list(tqdm(p.imap_unordered(calculate, data), total=len(data), position=0, desc='tests graphs'))
    calculate(None)